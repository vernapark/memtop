"""
Combined Web Server + Telegram Bot with Multi-Cloudinary Support
Runs both the video streaming website and telegram bot in one process
Now supports multiple Cloudinary accounts for increased storage
"""
import os
import json
import secrets
...
from multi_cloudinary_setup import (
    upload_video, get_videos, delete_video,
    get_accounts_info, add_account, toggle_account
)
...
async def main():
    ...
    print("   WITH MULTI-CLOUDINARY SUPPORT")
    print("=" * 70)
    ...
    app.router.add_post('/api/delete-video', delete_video)
    
    # Multi-Cloudinary account management routes
    app.router.add_get('/api/cloudinary-accounts', get_accounts_info)
    app.router.add_post('/api/cloudinary-accounts/add', add_account)
    app.router.add_post('/api/cloudinary-accounts/toggle', toggle_account)
    ...
    logger.info(f"   POST /api/upload-video -> upload_video")
    logger.info(f"   GET /api/videos -> get_videos")
    logger.info(f"   POST /api/delete-video -> delete_video")
    logger.info(f"   GET /api/cloudinary-accounts -> get_accounts_info")
    logger.info(f"   POST /api/cloudinary-accounts/add -> add_account")
    logger.info(f"   POST /api/cloudinary-accounts/toggle -> toggle_account")
    ...
    print(f"☁️  Multi-Cloudinary: Enabled")
    print("=" * 70)
