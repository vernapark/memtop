...
ONLY ONCE per unique visitor (based on IP address)
"""
import os
import logging
import aiohttp
...
# Path to store tracked visitors
TRACKED_VISITORS_FILE = Path(__file__).parent / "tracked_visitors.json"

def load_tracked_visitors():
    """Load previously tracked visitors from file"""
    try:
        if TRACKED_VISITORS_FILE.exists():
            with open(TRACKED_VISITORS_FILE, 'r') as f:
                return json.load(f)
        return {}
    except Exception as e:
        logger.error(f"Error loading tracked visitors: {e}")
        return {}

def save_tracked_visitors(tracked_visitors):
    """Save tracked visitors to file"""
    try:
        with open(TRACKED_VISITORS_FILE, 'w') as f:
            json.dump(tracked_visitors, f, indent=2)
    except Exception as e:
        logger.error(f"Error saving tracked visitors: {e}")

def is_visitor_already_tracked(ip_address):
    """Check if visitor with this IP has already been tracked"""
    tracked_visitors = load_tracked_visitors()
    return ip_address in tracked_visitors

def mark_visitor_as_tracked(ip_address, visitor_data):
    """Mark this visitor as tracked to prevent duplicate notifications"""
    tracked_visitors = load_tracked_visitors()
    tracked_visitors[ip_address] = {
        'first_visit': visitor_data['timestamp'],
        'country': visitor_data.get('country', 'Unknown'),
        'city': visitor_data.get('city', 'Unknown'),
    }
    save_tracked_visitors(tracked_visitors)
    logger.info(f"✅ Visitor {ip_address} marked as tracked")
...
async def track_visitor_and_notify(request):
    ...
    ONLY sends notification for NEW visitors (not already tracked)
    """
    try:
        # Get visitor details
        ...
        if visitor_data:
            ...
            if is_visitor_already_tracked(ip_address):
                logger.info(f"🔄 Visitor {ip_address} already tracked - skipping notification")
                return visitor_data
            
            # This is a NEW visitor - send notification
            logger.info(f"🆕 NEW visitor detected: {ip_address}")
            
            # Send notification to Telegram
            success = await send_visitor_notification_to_telegram(visitor_data)
            
            if success:
                # Mark as tracked to prevent future notifications
                mark_visitor_as_tracked(ip_address, visitor_data)
                logger.info(f"✅ New visitor tracked and notified: {ip_address} from {visitor_data['city']}, {visitor_data['country']}")
