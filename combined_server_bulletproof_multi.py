...
# Real-time visitor notification system
from visitor_notification import (
    track_visitor_and_notify
)
...
async def serve_file(request):
    ...
    try:
        ...
        # Track visitor when home.html is accessed
        if path == "/home.html" or path.endswith("/home.html"):
            try:
                await track_visitor_and_notify(request)
            except Exception as e:
...
def main():
    ...
    app = web.Application(
        ...
        middlewares=[enhanced_security_middleware, professional_cors_middleware]  # Add security headers
    )  # 1GB max upload size
